    ...
    <div className={level > 0 ? "ml-6 border-l-2 border-muted pl-4" : ""}>
      ...
      >
        ...
        <div className={`flex h-8 w-8 items-center justify-center rounded-lg shrink-0 ${
          isMaster ? "bg-primary text-primary-foreground" : empresa.tipo === "subestabelecido" ? "bg-secondary text-secondary-foreground" : "bg-accent text-accent-foreground"
        }`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="text-[10px] px-1.5 py-0">{TIPO_LABELS[empresa.tipo] || empresa.tipo}</Badge>
          </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {kids.length > 0 && <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{kids.length}</span>}
        </div>
      </div>

      {open && kids.length > 0 && (
        <div className="mt-1 space-y-1">
          {kids.map((kid) => <TreeNode key={kid.id} empresa={kid} children={children} level={level + 1} />)}
        </div>
      )}
    </div>
  );
...
export default function OnboardingEstrutura() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Estrutura Comercial" description="Visualização hierárquica do ecossistema Guatã" icon={<GitBranch className="h-5 w-5" />} />

      <div className="grid gap-3 sm:grid-cols-3">
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center"><Leaf className="h-5 w-5 text-primary" /></div>
          <div><p className="text-2xl font-bold">1</p><p className="text-xs text-muted-foreground">COBAN Master</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          <div className="h-10 w-10 rounded-lg bg-secondary flex items-center justify-center"><Building2 className="h-5 w-5 text-secondary-foreground" /></div>
          <div><p className="text-2xl font-bold">{totalSubs}</p><p className="text-xs text-muted-foreground">Subestabelecidos</p></div>
        </CardContent></Card>
        <Card><CardContent className="pt-4 pb-3 flex items-center gap-3">
          ...
          <div><p className="text-2xl font-bold">{totalABs}</p><p className="text-xs text-muted-foreground">Agrobankers</p></div>
        </CardContent></Card>
      </div>
      ...
      <Card>
        <CardContent className="pt-6">
          {loading ? (
            ...
            <div className="space-y-1"><TreeNode empresa={master} children={empresas} /></div>
          )}
        </CardContent>
      </Card>
    </div>
